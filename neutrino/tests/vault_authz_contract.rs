...
async fn reveal_audit_attributes_request_actor_not_system_happy_path() {
    ...
    let events = NeutrinoSecretAuditEvent::query(&v, valence::use_!(r"**Test:** Fixture **Neutrino Secret Audit Event** list for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("query audit");
