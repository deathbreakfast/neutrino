...
async fn last_audit_hash_for_secret(v: &Valence, secret_id: &str) -> NeutrinoResult<String> {
    let rows = NeutrinoSecretAuditEvent::query(v, valence::use_!(r"In **instrumentation**, we **list Neutrino Secret Audit Event** so the product can show or process the matching set for this workflow. Callers allowed for **instrumentation** use the list; it is not a public dump of every field to anonymous visitors."))
